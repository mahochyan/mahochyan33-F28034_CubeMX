...
(function () {
  ...
  async function run(q) {
    // Local results (Chinese keywords + registers + wizards) always work offline.
    const local = localResults(q);
    let remote = [];
    try {
      const res = await api(`/api/search?q=${encodeURIComponent(q)}&device=${encodeURIComponent(Store.device || 'TMS320F28034')}`);
      remote = (res.results || []).map(r => ({ kind: r.match === 'mux' ? 'mux' : 'pin', ...r }));
    } catch (err) {
      // search backend failure is isolated — still show local results.
      if (!local.length) {
        panelEl.innerHTML = `<div class="search-empty">搜索失败：${esc(err.message)}${err.status ? ' (HTTP ' + err.status + ')' : ''}</div>`;
        panelEl.classList.remove('hidden');
        return;
      }
    }
    items = dedup(local.concat(remote));
  ...
  function choose(it) {
    ...
    if (it.kind === 'wizard' && it.wizard) {
      if (window.Wizard) Wizard.open(it.wizard);
      activateMidTab('wizard');
      window.setStatus(`打开向导：${it.label}`);
      return;
    }
    if (it.kind === 'register') {
